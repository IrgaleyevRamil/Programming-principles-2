...
def create_coin():
    coin_type = random.choice([
        {"value": 1, "radius": 13, "color": YELLOW},
        {"value": 2, "radius": 17, "color": ORANGE},
        {"value": 3, "radius": 21, "color": PURPLE},
    ])

    coin_type["x"] = random.randint(road_left + 45, road_right - 45)
    coin_type["y"] = random.randint(-500, -50)

    return coin_type


def draw_background(enemy_speed):
    screen.fill(GREEN)
...
def draw_coin(coin, angle):
    ...
    pygame.draw.circle(glow, (255, 220, 0, 60), (35, 35), radius + 10)
    screen.blit(glow, (x - 35, y - 35))

    width = int(radius + math.sin(angle) * 5)

    pygame.draw.ellipse(screen, (210, 130, 0), (x - width, y - radius, width * 2, radius * 2))
    pygame.draw.ellipse(screen, color, (x - width + 3, y - radius + 3, width * 2 - 6, radius * 2 - 6))

    text = small_font.render(str(value), True, BLACK)
    screen.blit(text, (x - text.get_width() // 2, y - text.get_height() // 2))


def draw_score(coins, enemy_speed):
    score_box = pygame.Surface((190, 75), pygame.SRCALPHA)
    pygame.draw.rect(score_box, (0, 0, 0, 120), (0, 0, 190, 75), border_radius=12)
    screen.blit(score_box, (WIDTH - 205, 12))

    text = font.render(f"Coins: {coins}", True, WHITE)
    speed_text = small_font.render(f"Speed: {enemy_speed}", True, WHITE)

    screen.blit(text, (WIDTH - text.get_width() - 25, 20))
    screen.blit(speed_text, (WIDTH - speed_text.get_width() - 25, 52))
...
def game_over_menu(coins, best_score):
    ...
    while True:
        draw_background(BASE_ENEMY_SPEED)
        draw_road()
...
def run_game(best_score):
    ...
    while True:
        enemy_speed = BASE_ENEMY_SPEED + coins // N
